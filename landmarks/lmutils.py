...
def calc_landmark_nme(gt_lms, pred_lms, ocular_norm='pupil', image_size=None):
    ...
    def get_bb_from_landmarks(landmarks):
        bb = np.zeros([landmarks.shape[0], 4])
        min_lmk = np.min(landmarks, axis=1)
        max_lmk = np.max(landmarks, axis=1)
        bb[:, 0] = min_lmk[:, 0]
        bb[:, 1] = min_lmk[:, 1]
        bb[:, 2] = max_lmk[:, 0]
        bb[:, 3] = max_lmk[:, 1]
    ...
    if gt.shape[1] == 5:  # VGGFace2
    ...
    elif gt.shape[1] == 84:  # menpo
        ocular_dists = np.sqrt(np.sum((gt[:, 61] - gt[:, 52]) ** 2, axis=1))
    elif gt.shape[1] == 98:  # WFLW
        ocular_dists = np.sqrt(np.sum((gt[:, 72] - gt[:, 60]) ** 2, axis=1))
    elif gt.shape[1] == 68:  # 300-W and 300W-LP
        if ocular_norm == 'pupil':
            ...
            bbox = get_bb_from_landmarks(gt)
            left_corner = np.array([bbox[:, 0], bbox[:, 1]])
            right_corner = np.array([bbox[:, 2], bbox[:, 3]])
            dists = right_corner - left_corner
            ocular_dists = np.sqrt(dists[0]*dists[1])
    ...
    if pred.shape[2] == 3:  # 3D landmarks
        pred_mean_z = np.mean(pred[:, :, 2], axis=1).reshape(len(gt), 1)
        gt_mean_z = np.mean(gt[:, :, 2], axis=1).reshape(len(gt), 1)
        # print(pred_mean_z, gt_mean_z)
        pred[:, :, 2] -= (pred_mean_z - gt_mean_z)  # Remove offset on z coordinate
...
def calc_landmark_nme_var(gt_lms, pred_lms, ocular_norm='pupil', image_size=None):
    def reformat(lms):
        lms = to_numpy(lms)
        if len(lms.shape) == 2:
            lms = lms.reshape((1, -1, 2))
    ...
    norm_dists = np.sqrt(np.sum((gt - pred) ** 2, axis=2)) / ocular_dists.reshape(len(gt), 1)
    # print('norm_dists', norm_dists.shape)
    variance = np.var(norm_dists * 100, axis=1)

    return norm_dists * 100, variance
...
def calc_landmark_ssim_score(X, X_recon, lms, wnd_size=None):
    ...
    for i in range(nimgs):
        ...
        S = compare_ssim(input_images[i], recon_images[i], data_range=data_range, channel_axis=-1, full=True)[1]
        S = S.mean(axis=2)
        for lid in range(nlms):
            x = int(lms[i, lid, 0])
            y = int(lms[i, lid, 1])
