...
class FaceCrop():
    ...
    def calculate_crop_parameters(self, img, lms_orig):
        ...
        if self.crop_by_eye_mouth_dist:
            ...
            has_eyebrows = len(lms) != 68 or len(lms) != 21 or len(lms) != 84 and len(lms) != 98
            if not has_eyebrows:
                # height is
                h *= 1.5
                t = t - h / 2
    ...
    def apply_to_landmarks(self, lms_orig, pose=None):
        ...
        if lms_new.shape[1] == 3:
            lms_new[:, 2] = lms_new[:, 2] * self.output_size[1] / crop_height
    ...
    def apply_to_landmarks_inv(self, lms):
        ...
        if lms_new.shape[1] == 3:
            lms_new[:, 2] = lms_new[:, 2] * crop_height / self.output_size[1]
