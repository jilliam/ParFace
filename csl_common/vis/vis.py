...
def vis_square(data, padsize=1, padval=0, wait=0, nCols=10, title='results', dsize=None, fx=None, fy=None,
               normalize=False):
...
def add_landmarks_to_images(images, landmarks, color=None, radius=2, gt_landmarks=None,
    ...
    def draw_wireframe_lines_84(img, lms):
        pts = lms.reshape((-1, 1, 2)).astype(np.int32)
        cv2.polylines(img, [pts[:33]], isClosed=False, color=color, lineType=cv2.LINE_AA)  # head outline
        cv2.polylines(img, [pts[33:38]], isClosed=False, color=color, lineType=cv2.LINE_AA)  # left eyebrow
        cv2.polylines(img, [pts[38:43]], isClosed=False, color=color, lineType=cv2.LINE_AA)  # right eyebrow
        cv2.polylines(img, [pts[43:47]], isClosed=False, color=color, lineType=cv2.LINE_AA)  # nose vert
        cv2.polylines(img, [pts[47:52]], isClosed=False, color=color, lineType=cv2.LINE_AA)  # nose hor
        cv2.polylines(img, [pts[52:58]], isClosed=True, color=color, lineType=cv2.LINE_AA)  # left eye
        cv2.polylines(img, [pts[58:64]], isClosed=True, color=color, lineType=cv2.LINE_AA)  # right eye
        cv2.polylines(img, [pts[64:76]], isClosed=True, color=color, lineType=cv2.LINE_AA)  # outer mouth
        cv2.polylines(img, [pts[76:84]], isClosed=True, color=color, lineType=cv2.LINE_AA)  # inner mouth
    ...
    def draw_offset_lines(img, lms, gt_lms, errs):
        ...
        for i, (p1, p2) in enumerate(zip(lms, gt_lms)):
            if landmarks_to_draw is None or i in landmarks_to_draw:
                if p1.min() > 0:
                    cv2.line(img, tuple(p1.astype(int)), tuple(p2.astype(int)), colors[i], thickness=1,
    ...
    for img_id, (disp, lm) in enumerate(zip(new_images, landmarks)):
        if len(lm) in [68, 21, 19, 84, 98, 8, 5, 38]:
            if draw_dots:
                for lm_id in range(0, len(lm)):
                    if landmarks_to_draw is None or lm_id in landmarks_to_draw or len(lm) != 68:
                        ...
                        #     lm_color = (1,0,0)
                        cv2.circle(disp, tuple(lm[lm_id].astype(int).clip(0, disp.shape[0] - 1)), radius=radius,
                                   color=lm_color, thickness=-1, lineType=cv2.LINE_AA)
                        if lm_confs is not None:
                            ...
                            if lm_errs[img_id, lm_id] > 10.0:
                                cirle_color = (255, 0, 0)
                            cv2.circle(disp, tuple(lm[lm_id].astype(int)), conf_radius, cirle_color, 1,
                                       lineType=cv2.LINE_AA)
            ...
            if draw_wireframe:
                ...
                if nlms == 68:
                    ...
                    draw_wireframe_lines_84(disp, lm)
                elif nlms == 98:
...
def draw_z_vecs(levels_z, ncols, size, vmin=-1, vmax=1, vertical=False):
    ...
    z_grid_per_sample = [make_grid(all_vis_sample, nCols=len(levels_z)) for all_vis_sample in
                         zip(*z_vis_list_per_level)]
    return make_grid(z_grid_per_sample, nCols=ncols, normalize=False)


def draw_status_bar(text, status_bar_width, status_bar_height, dtype=np.float32, text_size=-1,
                    text_color=(255, 255, 255)):
    img_status_bar = np.zeros((status_bar_height, status_bar_width, 3), dtype=dtype)
    if text_size <= 0:
        text_size = status_bar_height * 0.025
    cv2.putText(img_status_bar, text, (4, img_status_bar.shape[0] - 5), cv2.FONT_HERSHEY_SIMPLEX, text_size, text_color,
                1, cv2.LINE_AA)
