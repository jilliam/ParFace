...
class AffectNet(facedataset.FaceDataset):
    ...
    def _load_annotations(self, split):
        ...
        path_annotations_mod = "../csv/affectnet.mod.pkl"
        if os.path.isfile(path_annotations_mod):
            ...
            annotations = pd.read_csv("../csv/affectnet_annotations.csv")
            print('done.')
    ...
    def __getitem__(self, idx):
        ...
        filename = sample.image_path    # filename = sample.subDirectory_filePath
        ...
        landmarks_to_return = self.parse_landmarks(sample.facial_landmarks[1:-1])
        landmarks_for_crop = landmarks_to_return if self.crop_source == 'lm_ground_truth' else None
        return self.get_sample(filename, bb, landmarks_for_crop,
                               landmarks_to_return=landmarks_to_return, landmarks_3d=None)
...
if __name__ == '__main__':
    ...
    dirs = cfg.get_dataset_paths('affectnet')
    train = True
    ...
    for data in dl:
        ...
        images = vis.to_disp_images(batch.images, denorm=False)
