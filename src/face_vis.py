...
def draw_results(X_resized, X_recon, levels_z=None, landmarks=None, landmarks_pred=None,
                 cs_errs=None, ncols=15, fx=0.5, fy=0.5, additional_status_text='', denorm=False):
    ...
    disp_X = vis.to_disp_images(X_resized, denorm=denorm)
    disp_X_recon = vis.to_disp_images(X_recon, denorm=denorm)
    ...
    for i in range(nimgs):
        ...
        ssim[i] = compare_ssim(disp_X[i], disp_X_recon[i], data_range=1.0, channel_axis=-1)
