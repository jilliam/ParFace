...
import src.face_vis as face_vis
...
class AAE(nn.Module):
    def __init__(self, input_size, output_size=None, pretrained_encoder=False,
        ...
        self.Q = resnet_ae.resnet18(pretrained=pretrained_encoder,
                                    ...
                                    layer_normalization=cfg.ENCODER_LAYER_NORMALIZATION).cuda(device)
        ...
        self.D_z = D_net_gauss(self.z_dim).cuda(device)
        self.D = Discriminator().cuda(device)
...
def vis_reconstruction(net, inputs, landmarks=None, landmarks_pred=None,
                       pytorch_ssim=None, fx=0.5, fy=0.5, ncols=10, denorm=False):
    net.eval()
    cs_errs = None
    with torch.no_grad():
        X_recon = net(inputs)
    ...
    return face_vis.draw_results(inputs_resized, X_recon, net.z_vecs(),
                                 ...
                                 cs_errs=cs_errs,
                                 fx=fx, fy=fy, ncols=ncols, denorm=denorm)
